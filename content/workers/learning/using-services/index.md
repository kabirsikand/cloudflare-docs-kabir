...
[Service bindings](/workers/platform/routing/service-bindings) are an API that facilitate Worker-to-Worker communication.

A Service binding allows you to send HTTP requests to another Worker without those requests going over the Internet. The request immediately invokes the downstream Worker, reducing latency as compared to a request to a third-party service. You can invoke other Workers directly from your code; Service bindings allow for much more composability on the Workers platform. In the example below, requests are validated by an authentication Workers Service.
...
Service bindings use the standard [Fetch API](/workers/runtime-apis/fetch). You can continue to use your existing utilities and libraries - a Service binding will trigger a `FetchEvent`. You can also change the environment of a Service binding, so you can test a new version of a Service.
...
![Unlike the typical microservice architecture, where services communicate over a network and can suffer from latency or interruption, Service bindings architecture is made to have no network delay or errors](./media/service-bindings-comparison.png)
